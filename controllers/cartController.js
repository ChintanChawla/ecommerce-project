...
exports.addInCart = async (req, res) => {
  ...
  try {
    const cart = await pool.query('SELECT * FROM cart WHERE user_id = $1 AND product_id = $2', [user_id, product_id]);
    console.log('check ')
...
exports.removeFromCart = async (req, res) => {
  ...
  try {
    const cartItem = await pool.query('SELECT * FROM cart WHERE user_id = $1 AND product_id = $2', [user_id, product_id]);
    console.log('check ')
